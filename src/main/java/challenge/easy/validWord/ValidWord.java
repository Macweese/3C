...
package challenge.easy.validWord;
...
class Solution
{
	public static boolean isValid(String s)
	{
		...
		for (char c : s.toCharArray()) {
			if (!Character.isLetterOrDigit(Character.toLowerCase(c))) return false;
			if (isVowel(c)) v = true; else if (Character.isLetter(c)) k = true;
		}
		return s.length() > 2 && v && k;
	}
	private static boolean isVowel(char c)
	{
		c = Character.toLowerCase(c);
