...
def _parse_by_nbsp_and_example_blocks(content_html: str) -> Tuple[str, List[Dict[str, Any]], str]:
	...
	while i < len(rest_nodes):
		...
		if _is_example_header_p(node):
			...
			if ex_block:
				...
				# Primary path: p tags with strong labels (newer problems)
				...
				# Fallback path: a single <pre> holding labeled lines (older problems like 998)
				if (in_html is None or out_html is None or exp_html is None):
					pre = ex_block.find("pre")
					if pre:
	...
	# 3) Constraints: use only the list, let template print the heading
	constraints_html = ""
