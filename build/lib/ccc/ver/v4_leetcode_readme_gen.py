...
def split_desc_examples_constraints(content_html: str) -> Tuple[str, List[Dict[str, Any]], str]:
	...
	- description_html has examples and constraints removed
	- examples is a list of dicts with number, images_html, input_html, output_html, explanation_html
	- constraints_html is the constraints list HTML (e.g., the UL/OL and header), empty if none
	"""
	if not content_html:
		return "", [], ""
	...
	# 1) Find constraints header and gather constraints block (header + next list)
	constraints_html = ""
	constraints_header_tag: Optional[Tag] = None
	for t in soup.find_all(True, recursive=True):
		if _is_constraints_header(t):
			# Ensure we pick the container for header (if strong/b inside a paragraph)
			constraints_header_tag = t.parent if t.name in ("strong", "b") and t.parent else t
			break
	if constraints_header_tag:
		# Look for the next UL/OL after the header container
		list_tag = constraints_header_tag.find_next(lambda tag: isinstance(tag, Tag) and tag.name in ("ul", "ol"))
		# Compose HTML
		parts = [str(constraints_header_tag)]
		if list_tag:
			parts.append(str(list_tag))
		constraints_html = "\n".join(parts)
		# Remove from soup to avoid duplication
		if list_tag:
			list_tag.decompose()
		constraints_header_tag.decompose()

	# 2) Extract example blocks with explicit "Example" headers
	examples: List[Dict[str, Any]] = []
	example_header_nodes: List[Tag] = []
	for t in soup.find_all(True, recursive=True):
		# Headers usually look like <p><strong>Example 1:</strong></p> or similar
		if isinstance(t, Tag):
			if t.name in ("p", "h2", "h3", "h4", "div"):
				# direct text or contains strong/b
				if EX_HEADER_TEXT_RE.match(t.get_text(strip=True) or ""):
					example_header_nodes.append(t)
				else:
					strong = t.find(["strong", "b"])
					if strong and EX_HEADER_TEXT_RE.match(strong.get_text(strip=True) or ""):
						example_header_nodes.append(t)

	# Helper to get block nodes from this header until next header or constraints (which we've removed)
	def collect_block_from_header(header_tag: Tag) -> List[Any]:
		nodes: List[Any] = []
		cur = header_tag
		nodes.append(cur)
		while True:
			...
			if not nxt:
				break
			# stop if next sibling is another example header
			if isinstance(nxt, Tag):
				if _is_constraints_header(nxt):
					break
				if EX_HEADER_TEXT_RE.match(nxt.get_text(strip=True) or ""):
					break
	...
	if example_header_nodes:
		for idx, header in enumerate(example_header_nodes):
			block_nodes = collect_block_from_header(header)
			# Build block HTML
			block_html = _html_of(block_nodes)
			# Extract labels content
			in_html = _extract_label_content_from_html(block_html, "Input")
			out_html = _extract_label_content_from_html(block_html, "Output")
			exp_html = _extract_label_content_from_html(block_html, "Explanation")
			# Collect images in the block
			block_soup = BeautifulSoup(block_html, "html.parser")
			imgs = [str(img) for img in block_soup.find_all("img")]
			examples.append({
				"number": str(idx + 1),
				"images_html": imgs,
				"input_html": in_html,
			...
			# Remove nodes from soup to prevent duplication
			for n in block_nodes:
				try:
					if isinstance(n, Tag) or isinstance(n, NavigableString):
						# Using id(n) guard to avoid double-remove
						if id(n) not in used_nodes:
							used_nodes.add(id(n))
							if isinstance(n, Tag):
								n.decompose()
							else:
	...
	# 3) Fallback: if no "Example" headers were found, detect <pre> blocks that contain labels
	if not examples:
		pre_tags = soup.find_all("pre")
		for i, pre in enumerate(pre_tags, start=1):
			pre_html = pre.decode_contents()
			if not re.search(r'(?is)<\s*(?:strong|b)[^>]*>\s*(?:Input|Output|Explanation)\s*:?\s*</\s*(?:strong|b)\s*>', pre_html):
				# If no explicit labels inside, skip (we don't want random pre blocks)
				continue
			in_html = _extract_label_content_from_html(pre_html, "Input")
			out_html = _extract_label_content_from_html(pre_html, "Output")
			exp_html = _extract_label_content_from_html(pre_html, "Explanation")
			# Try to include img directly preceding this pre within the same parent
			imgs: List[str] = []
			sib = pre.previous_sibling
			while sib and isinstance(sib, NavigableString) and str(sib).strip() == "":
	...
	# 4) Clean remaining soup (remove empty paragraphs like &nbsp;)
	for p in soup.find_all("p"):
		txt = p.get_text().strip().replace("\xa0", "")
		if txt == "":
			p.decompose()

	description_html = soup.decode_contents().strip()

	# 5) Post-process subsection HTMLs
	def _postprocess(html_fragment: Optional[str]) -> Optional[str]:
	...
	for ex in examples:
		ex["input_html"] = _postprocess(ex.get("input_html"))
		ex["output_html"] = _postprocess(ex.get("output_html"))
		ex["explanation_html"] = _postprocess(ex.get("explanation_html"))
