...
# ---------- Helpers for pattern-based parsing ----------
...
def _parse_by_nbsp_and_example_blocks(content_html: str) -> Tuple[str, List[Dict[str, Any]], str]:
	...
	Deterministically parse description, examples, and constraints using the site's structure:
	  Description
	  <p>&nbsp;</p>
	  (Example header p + example segment) repeated
	  <p>&nbsp;</p>
	  <p><strong>Constraints:</strong></p>
	  <ul>...</ul>
	An “example segment” is:
	  - Prefer a single <div class="example-block"> (modern shape), OR
	  - Otherwise all sibling nodes after the header up to the next Example header/Constraints/&nbsp; (older shape, e.g., header + img + pre)
	...
	def collect_example_segment(nodes: List[Tag], start_idx: int) -> Tuple[List[Tag], int]:
		"""
		Collect nodes after an example header up to next header/constraints/&nbsp;.
		Returns (segment_nodes, next_index)
		"""
		seg: List[Tag] = []
		j = start_idx + 1
		while j < len(nodes):
			cand = nodes[j]
			if _is_nbsp_paragraph(cand):
				break
	...
	def extract_from_block_html(block_html: str) -> Tuple[List[str], Optional[str], Optional[str], Optional[str]]:
		"""
		From a block of HTML (either example-block or the collected segment),
		extract images, Input, Output, Explanation.
		- Prefer p tags with strong labels (modern LC).
		- Fallback to a single pre with labeled lines (older/outlier LC like 998).
		"""
		...
		if p_input:
			# strip the <strong>Input:</strong> marker and return the remainder
		...
		if p_expl_label:
			# Explanation can span subsequent <p> siblings until another label starts
			exp_parts: List[str] = []
		...
		# Fallback: a single <pre> with labeled lines
	...
	# 2) From rest_nodes, collect example segments following each example header
	...
	while i < len(rest_nodes):
		...
		if _is_example_header_p(node):
			...
			# Prefer a contained <div class="example-block"> inside the segment
			ex_block = None
			...
			if ex_block is not None:
				block_html = str(ex_block)
			else:
				# Use the entire collected segment as the block (handles 998 style: img + pre)
				block_html = _html_of(segment_nodes)
	...
	# 3) Constraints: use only the list; let the template print the heading
	constraints_html = ""
